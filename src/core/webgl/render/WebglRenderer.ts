...
import { BaseTexture, TextTexture } from "../../texture";
...
export class WebglRenderer {
    ...
    private _renderElement<T extends Container>(transform: Transform, constructor: Constructor<T>, matrix: Matrix, callback: Function) {
        const element = transform.container;
        // console.log(element);
        if (!element || !element.texture) {
            return false;
        }
 ...
 * @param component 
 */
function drawBefore(program: WebGLProgram, matrix: Matrix, component: IShowComponent) {
    const drawTexture = component.texture;
